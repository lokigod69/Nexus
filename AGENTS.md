...
Nexus v2 is the capture layer of the Second Brain system: capture links/thoughts → AI enrich +
routing suggestion → one-tap confirm → `npm run pull` delivers into `<project>/memory/raw/`.
Canonical docs, in priority order: `scripts/acceptance.mjs` → `src/types/index.ts` → `docs/SPEC.md`.
Commands: `npm run dev` (port 3001), `npm run build`, `npm run pull`, `node scripts/acceptance.mjs`
(server must run with `NEXUS_LOCAL_DB=1` — .env.local carries LIVE prod Turso credentials).
